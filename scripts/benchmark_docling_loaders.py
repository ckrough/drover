...
def main() -> int:
    ...
    parser.add_argument(
        "--ground-truth", default="eval/ground_truth.jsonl", type=Path
    )
    parser.add_argument("--documents-dir", default="eval/samples", type=Path)
    parser.add_argument(
        "--out-json",
        type=Path,
        default=None,
    ...
    if payload.get("vendor_accuracy") is not None:
        line += f"vendor={payload['vendor_accuracy']:.3f} "
    if payload.get("date_accuracy") is not None:
        line += f"date={payload['date_accuracy']:.3f} "
